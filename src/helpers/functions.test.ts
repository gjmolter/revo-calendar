...
test('Feb 2000 had 29 days', () => {
  expect(helperFunctions.isLeapYear(1, 2000)).toStrictEqual([
    31,
    29,
    31,
    30,
...
test('Feb 2001 had 28 days', () => {
  expect(helperFunctions.isLeapYear(1, 2001)).toStrictEqual([
    31,
    28,
    31,
    30,
...
test('transparent RGBA is [0, 0, 0, 0]', () => {
  expect(helperFunctions.decomposeRGBA('transparent')).toStrictEqual([
    0,
    0,
    0,
    0
...
test('#0000FF0 RGBA is [0, 0, 255, 0]', () => {
  expect(helperFunctions.decomposeRGBA('#0000FF0')).toStrictEqual([
    0,
    0,
    255,
    0
...
test('#FFFFFF RGBA with half alpha is rgba(255, 255, 255, 0.5)', () => {
  expect(helperFunctions.getRGBAColorWithAlpha('#FFFFFF', 0.5)).toBe(
...
test('Date formatting works as expected', () => {
  ...
  ).toBe('Segunda-Feira, 07 de Dezembro de 2020')
})

test('Time formatting works as expected (24h)', () => {
  expect(
    helperFunctions.getFormattedTime(new Date(2020, 11, 7, 21, 41), true)
  ).toBe('21:41')
})

test('Time formatting works as expected (AM/PM)', () => {
  expect(
    helperFunctions.getFormattedTime(new Date(2020, 11, 7, 21, 41), false)
  ).toBe('9:41 PM')
})
