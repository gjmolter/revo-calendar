const helperFunctions = {
    ...
    decomposeRGBA: function (color: string) {
      ...
      if (color.indexOf('rgb') === -1) {
        ...
        if (temp_elem.style.color !== flag) return null
        temp_elem.style.color = color
        if (temp_elem.style.color === flag || temp_elem.style.color === '') return null
    ...
    getFormattedDate: function (date: Date, format: string, lang: string, languages: object) {
      var mm =
        date.getMonth() + 1 <= 9
          ? '0' + (date.getMonth() + 1)
